...
export class MemoBentoClient {
  ...
  async login(): Promise<void> {
    ...
    this.loginInFlight = (async () => {
      if (!this.opts.password) {
        throw new MemoBentoError(
          "인증이 켜진 서버입니다. MEMOBENTO_PASSWORD 를 설정하세요.",
          401,
        );
      }
      ...
      if (!res.ok) {
        ...
        throw new MemoBentoError(
          res.status === 401
            ? "MEMOBENTO_PASSWORD 가 서버 비밀번호와 다릅니다"
      ...
      if (res.status === 404) {
        throw new MemoBentoError(
          "이 서버에는 /api/login 이 없습니다. MemoBento 를 최신으로 올리세요.",
          404,
        );
      }
    })().finally(() => {
  ...
  private async request<T>(
    ...
    if (!res.ok) {
      ...
      throw new MemoBentoError(`${method} ${path} → ${res.status}: ${msg}`, res.status);
    }
