...
### 3.12 에이전트 기록은 감사 로그가 아니다

`agent_log` 는 MCP 로 들어온 **변경**만 남긴다. 판별은 요청 헤더 `x-mb-agent`
하나다 — 사람의 브라우저는 이걸 붙이지 않는다.

**위조를 막는 것이 목적이 아니다.** 헤더를 흉내낼 수 있는 쪽은 이미 API 를 통째로
쓸 수 있는 쪽이다. 목적은 "이 메모가 왜 바뀌었지" 를 나중에 되짚는 것이다.

읽기는 남기지 않는다. 목록 한 번 부를 때마다 줄이 쌓이면 정작 봐야 할 변경이 묻힌다.

기록은 각 API 라우트에서 명시적으로 부른다. `memo-server.ts` 안쪽에 넣으면 사람이
한 것과 구별할 수 없다 — 그쪽은 누가 불렀는지 모른다.

기록 실패는 삼킨다. 로그 때문에 메모 저장이 실패하면 주객이 뒤바뀐다.
