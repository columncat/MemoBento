...
**`Inbox` · `Voice`** — 밖에서 들어온 파일이 놓이는 두 자리입니다. `Inbox` 는 Discord 나
채팅창으로 보낸 첨부가 먼저 닿는 곳이고, `Voice` 는 자매 앱
[VoiceBento](https://github.com/columncat/VoiceBento) 가 전사할 소리·영상이 사는 곳입니다.
VoiceBento 는 파일을 따로 보관하지 않고 여기 놓인 것을 `/api/files/<id>` 로 읽습니다
(그 라우트가 `Range` 를 받으므로 재생 중 탐색이 됩니다). 접혀 있지 않고, 역시 이름
변경·삭제가 잠겨 있습니다.

**그 밖에**

- 삭제한 메모함·메모는 30일간 휴지통에 남고 되살릴 수 있습니다 (`설정 → 기록 → 휴지통`)
- MCP 로 붙은 에이전트가 무엇을 고쳤는지 남습니다 (`설정 → 기록 → 에이전트 기록`)
